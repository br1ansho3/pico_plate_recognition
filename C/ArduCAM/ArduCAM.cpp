...
void ArduCAM::InitCAM()
{
  ...
  switch (sensor_model)
  {
    case OV2640:
		...
		// TODO: set RGB configurations 
        else if (m_fmt == RGB) {
			// wrSensorRegs8_8(OV2640_RGB565);

			// wrSensorReg8_8(0xff, 0x01);
          	// wrSensorReg8_8(0x15, 0x00);
			// wrSensorRegs8_8(OV2640_RGB565_SHORT);
			// wrSensorRegs8_8(ov2640_settings_to_svga);
			wrSensorRegs8_8(OV2640_RGB565_1);
...
{
  	uint8_t value = 0;
	addr = addr& 0x7f;  // keep lower 7 bits? use? camera datasheet rules?
 	cbi(P_CS, B_CS);
